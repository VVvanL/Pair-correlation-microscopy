function [a,D,G,f]=Fitting_Anomalous_G0(tdata,ydata,SEM)
...
%To find the peaks in the fitted curve
%figure
%semilogx(tdata,ydata,'.')
%hold on
%l=fvec(theta,tplot);
...
%j=tplot;
%j=j';
%plot(j,l,'LineWidth',1.5,"Color",[1 0 0])
%findpeaks(l,j)  %function to find the peaks  
%grid off
%hold off
%[y_pk,x_pk]=findpeaks(l,j);
%x_pk
...
%parameters
a=theta(1);       %index of heterogeneity
D=theta(2);
G=theta(3);
Aoff = 0.000006;
...
y=Aoff + (G).*(1/(1+((4.*D.*tdata)/(w.^2))^a)).*(1/sqrt(1+((4.*D.*tdata)/((w.^2).*((z0/w).^2)))^a)).*exp(-(d^2)/((w.^2)+(4.*D.*tdata)));
end
